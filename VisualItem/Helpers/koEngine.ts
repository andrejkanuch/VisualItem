module Resco.Controls {
	export class KOEngine {
		...
		public render(model?: any, rootNode?: any): void {
			...
			this.m_templates.getKeys().forEach(key => document.write("<script type='text/html' id='" + key + "'>" + this.m_templates.getValue(key) + "</script>"));
			// register custom bindings
			this.m_customBindings.getKeys().forEach(key => ko.bindingHandlers[key] = this.m_customBindings.getValue(key));
			// render the model
